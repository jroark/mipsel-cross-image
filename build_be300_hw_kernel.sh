...
# VR4131 cache bug fix: ENABLED for real hardware.
# The VR4131 has a bug where Hit_Writeback_Inv_D doesn't work correctly.
# Split into separate Hit_Writeback_D + Hit_Invalidate_D operations.
# This matches the fix from patches/vr4131_cachebug_fix.patch (2.6 era).

# Fix flush_dcache_line: split combined op into writeback + invalidate
sed -i '/^static inline void flush_dcache_line/,/^}/ {
  s/cache_op(Hit_Writeback_Inv_D, addr);/cache_op(Hit_Writeback_D, addr);\n\tcache_op(Hit_Invalidate_D, addr);/
}' arch/mips/include/asm/r4kcache.h

# Fix protected_writeback_dcache_line: split for both EVA and non-EVA paths
...
if [ -f vmlinux ]; then
    ...
    ls -l vmlinux
    echo ""
    echo "Real hardware build. Test with:"
    echo "  Load linux-4.2.9/vmlinux onto BE-300 via CYACE bootloader"
    echo "  Kernel cmdline: console=tty0 console=ttyS0,9600 earlyprintk"
else
