...
Five donor-specific quirks the build script papers over:
...
- The donor's `launcher` (Alex Holden, 2002) panel layout is hardcoded — only 4-6 tiles fit vertically on the 240×320 LCD before the panel goes off-screen and the whole thing fails silently. Build-script `launcher.cnf` overlay caps at 5 entries (Clock, Tetris, Roach, SoftKbd, Tux). `$window_background_image` directive also paints the LCD teal and hides the panel; omit it.
- nxterm launched via launcher's `fork+execvp` (or via shell/setsid wrapper/double-fork) crashes at `pc=0 vaddr=0 TLBL` — the 2003 nxterm was probably only ever launched from `/bin/startx` on the original distro; the Terminal entry in the donor's stock cnf was aspirational. rcS launches a persistent nxterm in parallel as a guaranteed-working shell; Terminal is omitted from launcher's menu.
